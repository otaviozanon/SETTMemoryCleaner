...
namespace SETTMemoryCleaner
{
    ...
    public sealed class TrayIconContextMenuControl : ContextMenuStrip
    {
        ...
        public TrayIconContextMenuControl()
        {
            ...
            // Rounded border
            var windowCornerPreference = Constants.WinAPI.DesktopWindowManager.Value.WindowCornerPreferenceRound;
            NativeMethods.DwmSetWindowAttribute(Handle, Constants.WinAPI.DesktopWindowManager.Attribute.WindowCornerPreference, ref windowCornerPreference, sizeof(int));

            // Border color
            var borderColor = ColorTranslator.ToWin32(GetBorderColor());
            NativeMethods.DwmSetWindowAttribute(Handle, Constants.WinAPI.DesktopWindowManager.Attribute.BorderColor, ref borderColor, sizeof(int));
        }
